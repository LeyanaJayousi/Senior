...
class Residualblock(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(input,hidden,kernel_size=1,stride=1,padding=0,bias=False)
        ...
        self.conv3 = nn.Conv2d(hidden,hidden * self.ratio,kernel_size=1,stride=1,padding=0,bias=False,)
...
class ResNet(nn.Module):
    def __init__(self, Residualblock, layers, channels, num_classes):
        ...
        self.layer1 = self._reslayer(Residualblock, layers[0], output=64, stride=1)
        self.layer2 = self._reslayer(Residualblock, layers[1], output=128, stride=2)
        self.layer3 = self._reslayer(Residualblock, layers[2], output=256, stride=2)
        self.layer4 = self._reslayer(Residualblock, layers[3], output=512, stride=2)
    ...
    def _reslayer(self, Residualblock, blocknum, output, stride):
      ...
      if stride != 1 or self.input != output * 4:
            id = nn.Sequential(nn.Conv2d(self.input,output * 4,kernel_size=1,stride=stride,bias=False)
