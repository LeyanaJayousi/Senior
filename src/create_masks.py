...
if __name__ == "__main__":
    ...
    for index, row in df.iterrows():
        ...
        # Extract ROI coordinates and ClassId from the CSV
        x1, y1, x2, y2 = row['Roi.X1'], row['Roi.Y1'], row['Roi.X2'], row['Roi.Y2']

        class_id = row['ClassId']

        # Determine the shape type based on the class ID
        ...
        # Create the shape mask for the image
        mask = create_shape_mask(shape, image.shape)

        mask_filename = os.path.basename(image_path).replace(
            # Generating the file and saving results
            '.jpg', f'_{shape}_mask.jpg')
