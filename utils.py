...
def extract_20_classes(train_folder):
    ...
    all_folders = [folder for folder in os.listdir(paths.train_folder) if os.path.isdir(os.path.join(paths.train_folder, folder))]
...
def Train(model, criterion, optimizer, num_epochs, batch_size, dataloaders, out_path):
    ...
    for epoch in range(num_epochs):
        for phase in ['training', 'validation']:
            ...
            epoch_balanced_accuracy = balanced_accuracy_score(all_labels, all_preds)
...
def create_and_copy_folders(source_train_path, source_mask_path, destination_path):
    ...
    for subdir, _, files in os.walk(source_train_path):
        for file in files:
            if file.endswith(('png', 'jpg', 'jpeg')):
                shutil.copy(os.path.join(subdir, file), os.path.join(train_dir, f"Image{train_image_counter}.jpg"))
                train_image_counter += 1
    ...
    for subdir, _, files in os.walk(source_mask_path):
        for file in files:
            if file.endswith(('png', 'jpg', 'jpeg')):
                shutil.copy(os.path.join(subdir, file), os.path.join(mask_dir, f"Image{mask_image_counter}_label.png"))
                mask_image_counter += 1
